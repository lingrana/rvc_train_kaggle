...
from ultimate_rvc.security import download_https, safe_extract_zip, validate_remote_url
...
def download_file(url):
    validate_remote_url(url)
    file_name = pathlib.PurePosixPath(urlsplit(url).path).name or "downloaded_file"
    file_name = file_name.replace("/", "_").replace("\\", "_")
    download_https(url, pathlib.Path(zips_path) / file_name)
...
def extract(zipfile_path, unzips_path):
    try:
        with zipfile.ZipFile(zipfile_path, "r") as zip_ref:
            safe_extract_zip(zip_ref, pathlib.Path(unzips_path))
...
def unzip_file(zip_path, zip_file_name):
    ...
    with zipfile.ZipFile(zip_file_path, "r") as zip_ref:
        safe_extract_zip(zip_ref, pathlib.Path(extract_path))
