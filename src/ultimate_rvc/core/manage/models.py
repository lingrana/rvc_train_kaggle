...
from ultimate_rvc.security import download_https, safe_child, safe_extract_zip
...
def _extract_voice_model(
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zip_ref:
            safe_extract_zip(zip_ref, extraction_path)
        file_path_map = {
            ext: Path(root, name)
...
def download_voice_model(url: str, name: str) -> None:
    ...
    download_https(url, Path(zip_name))
...
def delete_models(
    ...
    for name in names:
        try:
            model_dir_path = safe_child(Path(directory), name)
        except ValueError:
            raise ModelNotFoundError(entity=entity, name=name) from None
