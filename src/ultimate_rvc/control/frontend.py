...
<div class="resume-picker hidden" id="resume-picker"><div class="resume-picker-box"><h2>选择恢复模型</h2><p class="resume-picker-copy">以下是当前 Kaggle 账号可访问的恢复 Dataset。选择后会下载 checkpoint，并在下一次训练时恢复。</p><div class="resume-list" id="resume-list"><div class="resume-empty">正在读取 Dataset...</div></div><div class="btns"><button class="btn btn-ghost" id="resume-cancel" type="button">取消</button></div><p id="resume-error"></p></div></div>
...
async function restoreSelectedDataset(handle){text($('#resume-error'),'');try{const setting=await(await api('/api/v1/kaggle-auth',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:'',resume_dataset:handle})})).json();setResumeControl(setting);const d=await(await api('/api/v1/resume',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({dataset:handle})})).json();$('#resume-picker').classList.add('hidden');notice('训练历史已准备：'+d.dataset+'。请使用相同模型名开始训练。')}catch(err){text($('#resume-error'),err.message)}}
async function openResumePicker(){if(!state.kaggleConfigured){await openKaggleSettings();text($('#kaggle-error'),'请先验证 Kaggle Token');return}$('#resume-picker').classList.remove('hidden');const list=$('#resume-list');list.innerHTML='<div class="resume-empty">正在读取 Dataset...</div>';text($('#resume-error'),'');try{const d=await(await api('/api/v1/resume/datasets')).json();if(!d.datasets||!d.datasets.length){list.innerHTML='<div class="resume-empty">没有找到以 -resume 结尾的恢复 Dataset</div>';return}list.innerHTML=d.datasets.map(item=>'<button class="resume-item" type="button" data-handle="'+escapeAttr(item.handle)+'"><strong>'+escapeHtml(item.title||item.handle)+'</strong><small>'+escapeHtml(item.handle)+(item.updated?' · 更新于 '+escapeHtml(item.updated):'')+'</small></button>').join('');list.querySelectorAll('.resume-item').forEach(item=>item.onclick=()=>restoreSelectedDataset(item.dataset.handle))}catch(err){list.innerHTML='<div class="resume-empty">读取失败</div>';text($('#resume-error'),err.message)}}
$('#resume-history').onclick=()=>openResumePicker().catch(err=>notice(err.message));
$('#resume-cancel').onclick=()=>$('#resume-picker').classList.add('hidden');
