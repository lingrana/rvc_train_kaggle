...
function _phaseLabel(snap,job){
  ...
  const m={uploading_file:'步骤1 · 上传音频',upload_validating:'步骤1 · 校验文件',preparing:'步骤2 · 准备环境',preprocessing:'步骤2 · 扫描文件',extracting:'步骤3 · 基频提取',extracting_pitch:'步骤3 · 基频提取',extracting_embed:'步骤3 · 特征提取',extracting_verify:'步骤3 · 特征校验',training:'步骤4 · 模型训练',indexing:'步骤4 · 生成索引',validating:'步骤4 · 验证模型',uploading:'步骤4 · 上传模型',completed:'步骤4 · 训练完成',stopped:'步骤4 · 已经停止',failed:'步骤4 · 执行失败'};
...
$('#kaggle-form').addEventListener('submit',async e=>{e.preventDefault();try{await submitKaggleToken($('#kaggle-token').value)}catch(err){setKaggleOwner('');text($('#kaggle-error'),err.message)}});
