...
function toggleSwitch(el){el.classList.toggle('on');const on=el.classList.contains('on');if(el.dataset.field==='detect-overtrain'){const t=$('#overtrain-threshold');if(t)t.disabled=!on}if(el.dataset.field==='f0-guidance'&&!on&&(state.selectedValues['pretrained']||'Default')==='Default'){setSel('pretrained','None')}}
